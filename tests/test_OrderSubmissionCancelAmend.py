...
def test_orders(Constants, bzx, DAI, LINK, accounts, web3, deploy_smart_wallet,LoanToken):
    ...
    orderStruct = main_contract.getOrderByOrderID(smart_wallet,1)
    tradeOrderStruct[0] = orderStruct[0] #ensure to put the trader as the address of the smart wallet
    tradeOrderStruct[4] = LINK.address #swap the loan and collateral tokens
    tradeOrderStruct[-4] = DAI.address
    tradeOrderStruct[2] = "10" #change fee amount used
    smart_wallet.amendActiveOrder(tradeOrderStruct,1,{'from':accounts[0]}) #amend order
    assert(main_contract.getOrderByOrderID(smart_wallet,1)[2] != "1")
    assert(main_contract.getOrderByOrderID(smart_wallet,1)[4] != DAI.address)
    smart_wallet.cancelOrder(1,{'from':accounts[0]}) #cancel order
    assert(main_contract.getTotalOrders(smart_wallet.address) == 0)
